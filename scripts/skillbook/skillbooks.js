...
var args = Args.create("skillbooks", "A simple script to help you find and use skill-granting items you didn't know you had.", {
  use: Args.flag({
    setting: "",
    default: false,
...
function main(command) {
  (0,external_kolmafia_namespaceObject.userNotify)("");
  ...
  if (args.buy) {
    if (args.buyLimit <= 0) {
      (0,external_kolmafia_namespaceObject.print)("Your buyLimit is set to 0, please set a real value and try again", "red");
      return;
    }
  ...
  var haveItems = skillGranters.filter(item => have(item));
  ...
  if (!args.buy) {
    ...
    haveItems.forEach(item => {
      (0,external_kolmafia_namespaceObject.print)("".concat(item, ", which gives the skill '").concat(item.skill, "'"));
    });
  }
}
...
function buyItems(items) {
  ...
  dontHave.forEach(item => {
    ...
    if (itemPrice > 0 && itemPrice < args.buyLimit) {
      ...
      if (args.sim) {
        (0,external_kolmafia_namespaceObject.print)("Sim: Buying 1 ".concat(item, " at ").concat(itemPrice));
      } else {
        (0,external_kolmafia_namespaceObject.buy)(1, item, args.buyLimit);
      }
    }
  });

  if (args.sim) {
    (0,external_kolmafia_namespaceObject.print)("You'd spend ".concat(totalPrice, " meat to get ").concat(skillCount, " new skill-granting items."), "green");
  }
}

function useReusable(inv) {
  var reusable = inv.filter(item => item.reusable);
  (0,external_kolmafia_namespaceObject.print)("Found ".concat(reusable.length, " reusable skill-granting items in your inventory that you haven't used..."), "green");
  useAll(reusable);
}
